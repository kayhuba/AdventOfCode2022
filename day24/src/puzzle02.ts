...
const findPath = (startPosition: Coords, endPosition: Coords, startMinute: number): ResultingPathElement[] => {
    ...
    openList.push(new Node(startPosition, startMinute, distance(endPosition, startPosition)));
    do {
...
linereader.eachLine("./input/input.txt", (line, last) => {
    ...
    if (last) {
        ...
        let endPosition: Coords = new Coords(fieldWidth, fieldHeight + 1);

        const reversedPathFirst = findPath(startPosition, endPosition, 0);
        const minutesFirst: number = reversedPathFirst[0].time;

        startPosition = new Coords(fieldWidth, fieldHeight + 1);
        endPosition = new Coords(1, 0);

        const reversedPathSecond = findPath(startPosition, endPosition, minutesFirst);
        const minutesSecond: number = reversedPathSecond[0].time;

        startPosition = new Coords(1, 0);
        endPosition = new Coords(fieldWidth, fieldHeight + 1);

        const reversedPathThird = findPath(startPosition, endPosition, minutesSecond);
        const minutesThird: number = reversedPathThird[0].time;

        console.log("Fewest minutes required to find the path there, back and there again: ", minutesThird);
    }
});
